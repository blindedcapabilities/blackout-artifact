 ...
 */
 ...
 static int seed = 0;
 static int nInputs = 1 << 10;
 static int nL1Neurons = 1 << 10;
...
void __attribute__((noinline)) Inference(long* input, long* output, long* W1, long* B1, long* W2, long* B2){
  long* L1Output = (long*) blinded_malloc(sizeof(long) * nL1Neurons);
  long* L2Output = (long*) blinded_malloc(sizeof(long) * nOutputs);
...
int main(){
  ...
  // Initialize inputs
  long* input = (long*) blinded_malloc(sizeof(long) * nInputs);
  for(int i = 0; i < nInputs; i++)
      input[i] = GenRandFloat(10);


  // Initialize outputs
  long* output = (long*) blinded_malloc(sizeof(long) * nOutputs);
